...
| `API_TOKEN` | To deploy | Main Makers API token. It stays in the Agent Runtime and mints a per-project temporary tenant token that is injected into direct sandbox CLI calls. Required for live deploys and credentialed local backends such as Blob; without it the agent still chats and previews a static frontend, but deploys fail. Do not commit it. |
